...
stm32_errors_t stm32_get(int fd, stm32_bootloader_info_t* info) {
	...
	if(serial_read(fd, &(info->version), 1) != SERIAL_ERR_OK)
		return STM32_ERR_SERIAL;

	len--;

	/* read supported commands */
	if(serial_read(fd, info->commands, len) != SERIAL_ERR_OK)
		return STM32_ERR_SERIAL;

	info->commandsSize = len;
...
stm32_errors_t stm32_get_prs(int fd, stm32_rpc_info_t* info){
	...
	if(serial_read(fd, &(info->rpdc), 1) != SERIAL_ERR_OK)
		return STM32_ERR_SERIAL;

	/* read protection enable counter */
	if(serial_read(fd, &(info->rpec), 1) != SERIAL_ERR_OK)
		return STM32_ERR_SERIAL;
...
//	static uint8_t id[0x100]; // 0xFF + 1
	...
	/* read device id */
	if(serial_read(fd, info->deviceId, len) != SERIAL_ERR_OK)
		return STM32_ERR_SERIAL;

	info->deviceIdSize = len;
